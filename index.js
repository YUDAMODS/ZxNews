...
//=================================================//
CFonts.say("SC PAIRING BY YudaMods", {
   font: 'chrome',
  align: 'left',
...
try{
if (sessionnye == "multi") {
let session = require("@whiskeysockets/baileys").useMultiFileAuthState
let { state, saveCreds } = await session("./sessionye")
var dataSesi = state
var saveSesi = saveCreds
} else if (sessionnye == "single") {
let session = require("@whiskeysockets/baileys").useSingleFileAuthState
...
if(usePairingCode && !zyren.authState.creds.registered) {
		const phoneNumber = await question(color('\n\n\nSilahkan masukin nomor Whatsapp Awali dengan 62:\n', 'magenta'));
		const code = await zyren.requestPairingCode(phoneNumber.trim())
...
let fgclink = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "62xxx@g.us"}, "message": {orderMessage: {itemCount: 9999999,status: 200, thumbnail: buffer, surface: 200, message: `${metadata.subject}`, orderTitle: 'memek', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
he = `HELLO 👋 SELAMAT DATANG DI GROUP ${metadata.subject} @${num.split("@")[0]}\n\n${metadata.desc}`
let link = `https://youtube.com/@YUDAMODS'`
let buttons = [
